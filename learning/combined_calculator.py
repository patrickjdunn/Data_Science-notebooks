...
#GDMT_status = 0 = Optimal, 1 = suboptimal, 2 = not on GDMT
...
# Blood pressure
hypertension = "No"  # "Yes" or "No"
hypertension_treatment = "No"  # "No", "Taking medications", "Making lifestyle changes"
...
def determine_ckm_stage(
        ...
        """
        Classify CKM Syndrome Stage (0–4) using AHA criteria.
        - metabolic_syndrome is derived from met_score >= 3
        """

        # Step 1: Calculate metabolic_syndrome
        metabolic_syndrome = "Yes" if met_score >= 3 else "No"

        # Step 2: Check for clinical CVD (Stage 4)
        has_clinical_cvd = any(x.strip().lower() == "yes" for x in [AMI, stroke_or_tia, PAD, PCI, CABG, heart_failure])
        if has_clinical_cvd:
            return 4

        # Step 3: Subclinical disease or high risk (Stage 3)
        if cvd_risk > 5:
            return 3

        # Step 4: Metabolic risk present (Stage 2)
        if (
            metabolic_syndrome == "Yes" or
            fasting_blood_sugar > 100 or
            (ckd_stage is not None and ckd_stage >= 2)
        ):
            return 2

        # Step 5: Adiposity only (Stage 1)
        if BMI >= 25 and metabolic_syndrome == "No" and fasting_blood_sugar <= 100 and cvd_risk <= 5:
            return 1

        # Step 6: Healthy (Stage 0)
        if BMI < 25 and metabolic_syndrome == "No" and fasting_blood_sugar <= 100 and cvd_risk <= 5:
            return 0
    ...
    #new
def get_cvd_risk(MLC_score, time_horizon, condition, risk_score):

        if MLC_score < 50:
            return 10  # Automatically high risk
        elif time_horizon.lower() == "10yr" and condition.lower() in ["cvd", "ascvd"]:
            return risk_score  # Use actual value
        else:
            return 0  # Default: low risk if not 10yr or not cvd/ascvd

    # ✅ Define input variables BEFORE calling the function
metabolic_syndrome = "Yes"
cvd_risk = 4.5
...
# Behavior-based care plan score
...
# Treatment targets
    ...
    diabetes_at_goal = 0 if A1c < 7 else 1
...
print("\n=== CarePlan Score ===")
#print(f"Care Plan Score: {cp_score}")
...
def calculate_cardiac_rehab_eligibility(
    ...
    """
    Determine eligibility for cardiac rehabilitation based on qualifying conditions.

    Parameters should be strings: "Yes" or "No"

    Returns:
    - eligibility (str): "Yes" if eligible, otherwise "No"
    """
